...
@Component({
  ...
  template: `
    <div class="page">
      <app-page-header title="ผู้ใช้" subtitle="เฉพาะผู้ใช้ระบบจัดการ">
        <p-button label="เพิ่มผู้ใช้" icon="pi pi-plus" (onClick)="create()" data-testid="users-new" />
      </app-page-header>
      <p-card>
      <div class="page-filters">
        <input pInputText placeholder="ค้นหา..." [(ngModel)]="search" (keyup.enter)="load()" data-testid="users-search" />
        <p-button label="ค้นหา" icon="pi pi-search" (onClick)="load()" data-testid="users-search-btn" />
      </div>
      ...
      @else if (!items().length) { <app-empty-state message="ไม่พบผู้ใช้" /> }
      @else {
        <p-table [value]="items()" [paginator]="true" [rows]="limit" [totalRecords]="total()" [lazy]="true" (onPage)="onPage($event)" [rowsPerPageOptions]="[10,20,50]">
          <ng-template pTemplate="header"><tr><th>ชื่อ</th><th>อีเมล</th><th class="col-fit">จัดการ</th></tr></ng-template>
          <ng-template pTemplate="body" let-row>
            <tr class="clickable-row" (click)="open(row._id)">
              <td>{{ row.displayName }}</td><td>{{ row.email }}</td>
...
export class UsersListComponent implements OnInit {
  ...
  load() {
    ...
    this.service.list({ page: this.page, limit: this.limit, search: this.search, isStaff: true }).subscribe({
  ...
  open(id: string) { void this.router.navigate(['/backoffice/users', id]); }
  create() { void this.router.navigate(['/backoffice/users', 'new']); }
  remove(id: string) {
    this.helper.confirm({
      message: 'ต้องการลบผู้ใช้นี้หรือไม่?',
      accept: () => this.service.remove(id).subscribe({
        next: () => { this.helper.toastSuccess('ลบผู้ใช้แล้ว'); this.load(); },
